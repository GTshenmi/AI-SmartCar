...
//    bufferPointer += sprintf(bufferPointer,"\n");
    ...
    //float angle =   (data->AIAngle / Servo.MaxAngle);
...
void SaveDataToEeprom(){
    ...
    for(int i = 0;i<MAX_LESENSOR_NUM;i++){
        EEPROMSetData(dataCounter++, &data->LESensorGain[i]);
    }
    for(int i = 0;i<MAX_LESENSOR_NUM;i++){
        EEPROMSetData(dataCounter++, &data->SESensorGain[i]);
    }
}
...
void LoadDataFromEeprom(){
    ...
    for(int i = 0;i<MAX_LESENSOR_NUM;i++){
        EEPROMGetData(dataCounter++, &data->LESensorGain[i]);
    }
    for(int i = 0;i<MAX_LESENSOR_NUM;i++){
        EEPROMGetData(dataCounter++, &data->SESensorGain[i]);
    }
}
