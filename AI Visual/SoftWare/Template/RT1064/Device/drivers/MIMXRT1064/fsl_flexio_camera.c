...
void FLEXIO_CAMERA_Init(FLEXIO_CAMERA_Type *base, const flexio_camera_config_t *config)
{
    ...
    shifterConfig.timerSelect   = base->timerIdx;  /*!< 定时器 0-3 */
    ...
    shifterConfig.pinSelect     = base->datPinStartIdx; /*!< FlexIO 位号(0-31) */
    shifterConfig.pinPolarity   = kFLEXIO_PinActiveHigh; /*!< FlexIO 极性 */
    shifterConfig.shifterMode   = kFLEXIO_ShifterModeReceive; /*!< 定义位移器工作模式(禁用/接收/发送/匹配存储/匹配连续) */
    shifterConfig.parallelWidth = FLEXIO_CAMERA_PARALLEL_DATA_WIDTH - 1U; /*!< 并行模式带宽 */
