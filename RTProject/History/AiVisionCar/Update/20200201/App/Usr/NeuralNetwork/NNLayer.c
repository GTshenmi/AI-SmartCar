...
void NNLayerInit()
{
    gMyCalculatorModel[0].weight = (double *)W0;
    ...
    gMyCalculatorModel[1].weight = (double *)W1;
    ...
    for(_indexLayer = 0 ; _indexLayer < LAYER_TOTAL ; _indexLayer++)
    {
        for(_indexW = 0 ; _indexW < model[_indexLayer].outDim ; _indexW++)
        {
            for(_indexX = 0 ; _indexX < model[_indexLayer].inDim ; _indexX++)
            {
                if(_indexLayer == 0)
                {
                    model[_indexLayer].outVal[_indexW] += (double)(x[_indexX] * _getWeight((double *)model[_indexLayer].weight,_indexX,model[_indexLayer].outDim,_indexW));
                }
                else
                {
