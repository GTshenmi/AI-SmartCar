...
{
    ADC_InitConfig(ADC0, 50000); // 初始化,采样率80kHz
    ADC_InitConfig(ADC1, 50000); // 初始化
    ADC_InitConfig(ADC2, 50000); // 初始化
    ADC_InitConfig(ADC3, 50000); // 初始化
    ADC_InitConfig(ADC7, 50000); // 初始化   如果使用龙邱母板  则测分压后的电池电压，具体可以看母板原理图
}
/*************************************************************************
...
void TFT_Show_EleMag_Info(void)
{
    ...
    sprintf(txt, "0:%03d %04d %04d ", Lleft1, LnowADC[0], ad_max[0]);
    TFTSPI_P8X16Str(0, 2, txt, u16WHITE, u16BLACK);
    sprintf(txt, "1:%03d %04d %04d ", Lleft2, LnowADC[1], ad_max[1]);
    TFTSPI_P8X16Str(0, 3, txt, u16WHITE, u16BLACK);
    sprintf(txt, "2:%03d %04d %04d ", Lright2, LnowADC[4], ad_max[4]);
    TFTSPI_P8X16Str(0, 4, txt, u16WHITE, u16BLACK);
    sprintf(txt, "3:%03d %04d %04d ", Lright1, LnowADC[5], ad_max[5]);
    ...
    BatVolt = ADC_Read(ADC7);  // 电感采集里面刷新电池电压
