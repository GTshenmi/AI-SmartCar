...
def LoadAndSelectData(ad_batch_size):
    ...
    train_label = np.load('./angle_train_label.npy')
    test_label = np.load('./angle_test_label.npy')
...
def CreateModelDense(num_classes=1, drop=0.25, isBN=True,ad_batch_size=1):
    ...
    model = Sequential()
    # -----------------
    model.add(Flatten(input_shape = (ad_batch_size,n_input,1)))
    model.add(Dense(96))  # 这个input_dim和input_shape一样，就是少了括号和逗号
    model.add(Activation('relu'))
    if isBN:
        model.add(BatchNormalization())
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        """)
    parser.add_argument(
        '-ad', '--ad_size', type=int, default=32,
        help="""\
            batch size
            """)
    parser.add_argument(
    ...
    x_train = x_train.reshape(int(x_train.size / ad_size / 8), ad_size, 8, 1)
    x_test = x_test.reshape(int(x_test.size / ad_size / 8), ad_size, 8, 1)
    ...
    x_test = x_train.astype('float16')
    y_test = y_train.astype('float16')
...
#plt.plot(range(len(y_test)), y_test, ls='-.',lw=2,c='r',label='real')
#plt.plot(range(len(pred_test_y)), pred_test_y, ls='-',lw=2,c='b',label='pred')
plt.plot(range(150), y_test[0:150], ls='-.',lw=2,c='r',label='real')
plt.plot(range(150), pred_test_y[0:150], ls='-',lw=2,c='b',label='pred')
# 绘制网格
plt.grid(alpha=0.4, linestyle=':')
plt.legend()
plt.xlabel('number') #设置x轴的标签文本
plt.ylabel('error') #设置y轴的标签文本
