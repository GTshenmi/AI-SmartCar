...
void icm_spi_w_reg_byte(uint8 cmd, uint8 val)
{
    ...
    SPIx.ReadWriteBytes(&SPI_Resources[2].SPIN,dat,NULL,2,0);
    ICM20602CS(1);
}
...
void icm_spi_r_reg_byte(uint8 cmd, uint8 *val)
{
    ...
    SPIx.ReadWriteBytes(&SPI_Resources[2].SPIN,dat,dat,2,0);
...
{
    ICM20602CS(0);
    SPIx.ReadWriteBytes(&SPI_Resources[2].SPIN,val,val,num,0);
    ICM20602CS(1);
