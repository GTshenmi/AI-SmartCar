...
if __name__ == "__main__":
    ...
    for a in dat_str:
        if (len(a) > 0):
            ...
            dat_int.append(a)  # 将数据转换为十进制并添加到dat_int列表
    ...
    # 计算有多少组完整数据
    len = int(int(len(dat_int) / 13) * 13)
    # 这是一步多余的操作
    dat_int = dat_int[0:len]

    # 将数据变形为矩阵 也可以说是二维数组
    dat_int = dat_int.reshape(int(dat_int.size / 13), 13)
    ...
    for a in dat_int:
        ...
        angle = a[11]
        # 将获取到的数值添加到list
        list.append([l, ml, m, mr, r, fl, fr, f8, angle])
    # 将list转换为数组 并将数据类型转换为有符号8位
    #list = (np.array(list)).astype(np.int8)
    list = (np.array(list)).astype(np.float16)
    # 通过切片操作 获得ad数据
    ad_dat = list[:, [0, 1, 2, 3, 4, 5, 6,7]]
    # 通过切片操作 获得pwm数据
    angle_dat = list[:, 8]
    # 新建一个ad数据
    dbg_ad = ad_dat
    ...
    f8 =ad_data_denoise((list[:, 7]))
    angle_dat = list[:, 8]
    # 转换数据类型为8位有符号数据 并通过ad_data_denoise函数滤波
    angle_dat = ad_data_denoise(angle_dat.astype('float16'))
    #angle_dat=angle_dat.astype('float32')
    # 将切片对象按列连接
    ad_dat = np.c_[a_l, a_ml, a_m, a_mr, a_r, a_fl, a_fr,f8]
    # 转换为有符号 不明白为啥要这样做 前面不是转换好了？
    ad_dat = ad_dat.astype('float32')

    # 扩展AD数据  实际上就是复制数据
    ad_dat = ad_array_extension(ad_dat, batch_size)
    # 最外将数据转换为三维数组 层与有多少组ad数据一致 中间层有参数指定 最内层为7个电感数据
    ad_dat = ad_dat.reshape(ad_dat.shape[0], batch_size, 8)

    origin_ad = ad_dat.astype('float32')
    origin_angle = angle_dat.astype('float32')
    ...
    # print(filename)
    # 准备输出文件的名称
    angle_fname = 'origin_angle_' + filename + '.npy'
    ad_fname = 'origin_ad_' + filename + '.npy'
    np.save(angle_fname, origin_angle)
    np.save(ad_fname, origin_ad)

    print("Generate ad.npy shape:", ad_dat.shape)
    print("Generate angle.npy shape:", angle_dat.shape)
    print('ad data shape:%f~%f' % (max(origin_ad.flatten()), min(origin_ad.flatten())))
    print('angle data shape:%f~%f' % (max(origin_angle), min(origin_angle)))

    print('Generate %s,%s' % (angle_fname, ad_fname))
