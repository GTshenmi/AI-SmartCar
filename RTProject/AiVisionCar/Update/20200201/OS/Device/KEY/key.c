...
uint8_t KEY_Scan(struct key *self)
{
    if(!self->KeyShield)
    {
        ...
        if(self->Event == KEY_UP && (self->PressedCallBack != NULL))
             self->PressedCallBack(self,self->UsrData);
        else if(self->Event == KEY_LONG && (self->LongPressedCallBack != NULL))
              self->LongPressedCallBack(self,self->UsrData);
    }
    else
    {
        self->State = KEY_NULL;
...
void KEY_BindUsrData(struct key *self,void *data)
{
    self->UsrData = data;
}

void KEY_DefaultCallBack(struct key *self,void *data)
{
