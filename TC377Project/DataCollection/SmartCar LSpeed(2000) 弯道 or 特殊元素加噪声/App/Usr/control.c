...
void HowToNameThisFunc2(data_t *data)
{
    ...
    switch(trackingstate)
    {
        ...
        case DeviateStraight:
            ...
            if(setTime <= 0.0)
            {
                ...
                deviateTime = random(1.0,3.0) * 50.0;//time/100.0f*0.7f;//random(0.1,0.7) * 50.0;

                noiseRange = random(0.0,35.0);

                noiseRange = ConstrainFloat(noiseRange,0.0,35.0);
        ...
        case ReturnStraight:
            ...
            {
                data->Bias += random(-noiseRange,noiseRange);
            }
            break;
    }
